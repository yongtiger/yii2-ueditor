		...
		for (var i = 0,img; img = images[i++];) {
			...
			for (var j = 0,url; url = imageUrls[j++];) {
				if (src.indexOf(url.original.replace(" ","")) != -1) {
					...
					img.setAttribute("title",'文件名:'+url.title+', 类型:'+url.type+', 文件大小:'+url.size+'(字节)');
					img.setAttribute('alt', url.original || '');

                    domUtils.removeAttributes(img, ["word_img","style","width","height"]);
					editor.fireEvent("selectionchange");

					///[UEditor_Event]
					editor.fireEvent('afterwordimage', {"src":img.src, "title":url.title || '', "alt":img.alt || '', "type":url.type || '', "size":url.size});
